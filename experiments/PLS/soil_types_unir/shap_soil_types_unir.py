...
data_complete = pd.read_csv(f'{workspace_root}/real_datasets/xrf/soil_types_unir.csv', sep=';') # local copy of Toledo and Guarapuava soil datasets
data = data_complete.loc[:, '1351.542978':'2150.472084']
...
XA_cal, XA_pred = ks.train_test_split(data_A.loc[:, '1351.542978':'2150.472084'], test_size=0.30) # class A
XA_cal = XA_cal.reset_index(drop=True)
XA_pred = XA_pred.reset_index(drop=True)

XB_cal, XB_pred = ks.train_test_split(data_B.loc[:, '1351.542978':'2150.472084'], test_size=0.30) # class B
...
shap_global_importance.to_csv('shap_soil_types_unir.csv', index=False, sep=';')